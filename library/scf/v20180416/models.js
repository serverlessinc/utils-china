...
class CreateFunctionRequest extends AbstractModel {
  ...
  deserialize(params) {
    ...
    if (params.DeadLetterConfig) {
      const obj = new DeadLetterConfig()
...
注意：此字段可能返回 null，表示取不到有效值。
  ...
  deserialize(params) {
    ...
    if (params.EipConfig) {
      const obj = new EipOutConfig()
      obj.deserialize(params.EipConfig)
      this.EipConfig = obj
    }

    if (params.AccessInfo) {
      const obj = new AccessInfo()
    ...
    if (params.PublicNetConfig) {
      const obj = new PublicNetConfigOut()
...
class GetFunctionLogsResponse extends AbstractModel {
  ...
  deserialize(params) {
    ...
    if (params.SearchContext) {
      const obj = new LogSearchContext()
